...
import { View,  Image, StyleSheet, TouchableWithoutFeedback, Keyboard, KeyboardAvoidingView, Platform } from "react-native";
  ...
  return (
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      ...
      <View style={styles.container}>
        ...
        {!!error && (
          <Text style={{ textAlign: "center", color: theme.colors.error, fontSize: theme.fontSizes.body }}>
            {error}
          </Text>
        )}
        ...
        {!userId && (
          <Button
            mode="text"
            onPress={() => {router.replace(`/(auth)/register?invite_id=${inviteId}`)}}
            style={{ marginTop: 24 }}
          >Don't have an account? Click here!</Button>
        )}
      </View>
      </KeyboardAvoidingView>
